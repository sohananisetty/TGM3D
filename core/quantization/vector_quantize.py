...
class VectorQuantize(nn.Module):
    def __init__(
        ...
        if not exists(sync_codebook) or sync_codebook is True:
            sync_codebook = (
                distributed.is_initialized() and distributed.get_world_size() > 1
            )

            sync_kmeans = sync_codebook
            sync_affine_param = sync_codebook

        print("Sync is turned on", sync_codebook)
    ...
    def forward(
        ...
        if self.training:
            ...
            if self.sync_update_v > 0.0:
                ...
                quantize = (
                    quantize
                    + (self.sync_update_v * quantize)
                    + (-self.sync_update_v * quantize).detach()
                )
