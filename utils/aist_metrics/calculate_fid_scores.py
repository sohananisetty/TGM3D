...
def axis_angles_from_6D(motions):
...
def visualize(motion, smpl_model):
    ...
    bbox_center = (
        keypoints3d.reshape(-1, 3).max(axis=0) + keypoints3d.reshape(-1, 3).min(axis=0)
    ) / 2.0
    bbox_size = keypoints3d.reshape(-1, 3).max(axis=0) - keypoints3d.reshape(-1, 3).min(
        axis=0
    )
...
def recover_motion_to_keypoints(motion, smpl_model, mode="9d"):
    if mode == "9d":
        smpl_poses, smpl_trans = recover_to_axis_angles(motion)
    elif mode == "6d":
        smpl_poses, smpl_trans = axis_angles_from_6D(motion)
...
def calculate_frechet_feature_distance(feature_list1, feature_list2):
    ...
    mean2 = np.mean(feature_list2, axis=0)
    std2 = np.std(feature_list2, axis=0) + 1e-10

    feature_list1 = (feature_list1 - mean) / std
    feature_list2 = (feature_list2 - mean2) / std2
...
if __name__ == "__main__":
    ...
    real_features = {
        "kinetic": [
            np.load(f) for f in glob.glob("./data/aist_features/*_kinetic.npy")
        ],
    ...
    for result_file in tqdm.tqdm(result_files):
        ...
        keypoints3d = recover_motion_to_keypoints(
            result_motion[:, 120:], smpl, mode="6d"
        )
