...
FLAGS = flags.FLAGS
flags.DEFINE_string(
    "anno_dir", "/mnt/data/aist_plusplus_final/", "Path to the AIST++ annotation files."
)
flags.DEFINE_string("audio_dir", "/mnt/data/AIST/music/", "Path to the AIST wav files.")
flags.DEFINE_string(
    "audio_cache_dir",
    "./data/aist_audio_feats/",
    "Path to cache dictionary for audio features.",
)
flags.DEFINE_enum(
    "split", "testval", ["train", "testval"], "Whether do training set or testval set."
)
flags.DEFINE_string(
    "result_files",
    "/mnt/data/aist_paper_results/*.pkl",
    "The path pattern of the result files.",
)
flags.DEFINE_bool("legacy", True, "Whether the result files are the legacy version.")
...
def recover_to_axis_angles(motion):
    ...
    axis_angles = (
        R.from_matrix(rotmats.reshape(-1, 3, 3))
        .as_rotvec()
        .reshape(batch_size, seq_len, 24, 3)
    )
...
def motion_peak_onehot(joints):
    ...
    peak_idxs = scisignal.argrelextrema(
        envelope, np.less, axis=0, order=10
    )  # 10 for 60FPS
...
def main(_):
    ...
    for i, seq_name in enumerate(seq_names):
        ...
        audio_feature = np.load(
            os.path.join(FLAGS.audio_cache_dir, f"{audio_name}.npy")
        )
        audio_beats = audio_feature[
            : keypoints3d.shape[0], -1
        ]  # last dim is the music beats
